...
class Agent:
    def __init__(self, action_space, num_of_states, alpha=0.5, gamma=0.99, num=6):
    ...
    def get_discrete_state(self, observation):
        ...
        discrete = [
            ...
            np.digitize(cart_v, bins=np.linspace(-2.0, 2.0, self.num)[1:-1]),
            np.digitize(pole_angle, bins=np.linspace(-.21, .21, self.num)[1:-1]),
            np.digitize(pole_v, bins=np.linspace(-2.0, 2.0, self.num)[1:-1]),
        ]
    ...
    def q_learning(self, observation, action, reward, next_observation):
        ...
        self.q_table[observation_state, action] += self.alpha * (reward + self.gamma * max_q_value - self.q_table[observation_state, action])

    @staticmethod
    def get_reward(done, step, max_step):
        if done:
            if step < max_step:
...
for i in range(1000):
    ...
    if max_step < step:
        ...
        max_step = step
env.close()
save_as_video(best_frames, max_step)
